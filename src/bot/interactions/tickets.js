...
async function onOpenTicket(interaction) {
...
async function onClaimTicket(interaction) {
...
async function onCloseTicket(interaction) {
  const ticket = getTicketByChannel(interaction.channelId);
  if (!ticket || ticket.status !== 'open') {
    await interaction.reply({ content: 'This is not an open ticket channel.', flags: EPHEMERAL_FLAG });
    return true;
...
const ticketButtonHandlers = {
  [OPEN_ID]: onOpenTicket,
  [CLAIM_ID]: onClaimTicket,
  [CLOSE_ID]: onCloseTicket
};

export async function handleTicketButton(interaction) {
  if (!interaction.guild) return false;
  const handler = ticketButtonHandlers[interaction.customId];
