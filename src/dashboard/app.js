...
export function createDashboard({ client }) {
  ...
  app.get('/dashboard/:guildId', ensureAuth, (req, res) => {
    ...
    res.send(`
      ...
      <p><a href="/dashboard/${guildId}/embed">Open Embed Creator</a> | <a href="/dashboard/${guildId}/automod">Automod Setup</a> | <a href="/dashboard/${guildId}/tickets">Ticketing Setup</a></p>
  ...
  app.get('/dashboard/:guildId/automod', ensureAuth, (req, res) => {
    ...
    res.send(`
      <h1>Automod Setup: ${guildId}</h1>
      <p><a href="/dashboard/${guildId}">Back to settings</a></p>

      <form method="post" action="/api/guilds/${guildId}/automod/settings">
        <label>Enable automod <input type="checkbox" name="automod_enabled" ${settings.automod_enabled ? 'checked' : ''} /></label><br /><br />
        <label>Block links <input type="checkbox" name="block_links" ${settings.block_links ? 'checked' : ''} /></label><br /><br />
        <label>Block invites <input type="checkbox" name="block_invites" ${settings.block_invites ? 'checked' : ''} /></label><br /><br />
        <label>Max mentions
          <input type="number" name="max_mentions" min="1" max="20" value="${settings.max_mentions || 5}" />
        </label><br /><br />
        <button type="submit">Save Automod Settings</button>
      </form>

      <h2>Blocked Words</h2>
      <p>${words.length ? words.join(', ') : 'No custom words added yet.'}</p>
      <form method="post" action="/api/guilds/${guildId}/automod/words/add">
        <label>Add word</label><br />
        <input name="word" required /><button type="submit">Add</button>
      </form>

      <form method="post" action="/api/guilds/${guildId}/automod/words/remove">
        <label>Remove word</label><br />
        <input name="word" required /><button type="submit">Remove</button>
      </form>
    `);
  });
